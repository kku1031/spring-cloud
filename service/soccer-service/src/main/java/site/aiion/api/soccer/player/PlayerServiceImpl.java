...
public class PlayerServiceImpl implements PlayerService {
        ...
        public Messenger findById(PlayerModel playerModel) {
                ...
                Optional<Player> entity = playerRepository.findById(playerId);
                if (entity.isPresent()) {
                        Player player = entity.get();
                        PlayerModel model = entityToModel(player);
        ...
        public Messenger update(PlayerModel playerModel) {
                Long playerId = playerModel.getId();
                Optional<Player> optionalEntity = playerRepository.findById(playerId);
                if (optionalEntity.isPresent()) {
                        Player existing = optionalEntity.get();
                        Team team = playerModel.getTeam_uk() != null
                                        ? teamRepository.findByTeam_uk(playerModel.getTeam_uk())
                                                        .orElse(existing.getTeam())
                                        : existing.getTeam();

                        Player updated = Player.builder()
                                        .id(existing.getId())
                                        .player_uk(playerModel.getPlayer_uk() != null ? playerModel.getPlayer_uk()
                                                        : existing.getPlayer_uk())
                                        .player_name(playerModel.getPlayer_name() != null ? playerModel.getPlayer_name()
                                                        : existing.getPlayer_name())
                                        .e_player_name(playerModel.getE_player_name() != null
                                                        ? playerModel.getE_player_name()
                                                        : existing.getE_player_name())
                                        .nickname(playerModel.getNickname() != null ? playerModel.getNickname()
                                                        : existing.getNickname())
                                        .join_yyyy(playerModel.getJoin_yyyy() != null ? playerModel.getJoin_yyyy()
                                                        : existing.getJoin_yyyy())
                                        .position(playerModel.getPosition() != null ? playerModel.getPosition()
                                                        : existing.getPosition())
                                        .back_no(playerModel.getBack_no() != null ? playerModel.getBack_no()
                                                        : existing.getBack_no())
                                        .nation(playerModel.getNation() != null ? playerModel.getNation()
                                                        : existing.getNation())
                                        .birth_date(playerModel.getBirth_date() != null ? playerModel.getBirth_date()
                                                        : existing.getBirth_date())
                                        .solar(playerModel.getSolar() != null ? playerModel.getSolar()
                                                        : existing.getSolar())
                                        .height(playerModel.getHeight() != null ? playerModel.getHeight()
                                                        : existing.getHeight())
                                        .weight(playerModel.getWeight() != null ? playerModel.getWeight()
                                                        : existing.getWeight())
                                        .team_uk(playerModel.getTeam_uk() != null ? playerModel.getTeam_uk()
                                                        : existing.getTeam_uk())
                                        .team(team)
                                        .build();
                ...
                List<Player> players = playerRepository.findAll().stream()
                                .filter(p -> p.getPlayer_name() != null &&
                                                (p.getPlayer_name().contains(keyword) ||
                                                                (p.getE_player_name() != null && p
                                                                                .getE_player_name().contains(keyword))
                                                                ||
                                                                (p.getNickname() != null
                                                                                && p.getNickname().contains(keyword))))
                                .collect(Collectors.toList());
